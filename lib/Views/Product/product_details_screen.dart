...
class ProductDetailsScreen extends ConsumerWidget {
  ...
  const ProductDetailsScreen({Key? key, required this.analysis})
    : super(key: key);

  //'Safe', 'Low_risk', 'Moderate_risk', or 'High_risk'
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Use analysis.id for matching, handle both cases (with and without ID)
    final currentProduct = products.isNotEmpty
        ? (analysis.id != null
            ? products.firstWhere(
                (p) => p.id == analysis.id,
                orElse: () => analysis,
            ...
            : products.firstWhere(
                (p) => p.title == analysis.title && p.description == analysis.description,
                orElse: () => analysis,
              ))
        : analysis;
        ...
        body: SingleChildScrollView(
          child: Column(
            children: [
              ...
              if (currentProduct.image != null && currentProduct.image!.isNotEmpty)
                Stack(
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        image: DecorationImage(
                          image: currentProduct.image!.startsWith('http')
                              ...
                              : FileImage(File(currentProduct.image!)) as ImageProvider,
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
                    ...
                    Positioned(
                      ...
                      child: IconButton(
                        ...
                        onPressed: () async {
                          if (currentProduct.id == null || currentProduct.id!.isEmpty) {
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Please wait for the product to be saved before marking as favorite'),
                          ...
                          try {
                            ...
                            if (result.isError && context.mounted) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(newStatus ? 'Removed from favorites' : 'Added to favorites'),
                                  ...
                                  showDialog(
                                    ...
                                    builder:
                                        (context) => AlertDialog(
                                          ...
                                          title: Center(child: Text('Risk')),
                                          content: Text(
                                            ingredient.risk.toString(),
                                            style: GoogleFonts.inter(
                                              fontSize: 14.sp,
                                            ),
                                          ),
